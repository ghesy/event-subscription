...
static void cb_state(pa_context* context, void* mainloop);
static void cb_subscribe(pa_context *c, pa_subscription_event_type_t ev, uint32_t idx, void *userdata);
static void cb_server_info(pa_context *c, const pa_server_info *i, void *userdata);
static void cb_sink_info(pa_context *c, const pa_sink_info *i, int eol, void *userdata);
...
pulse_ev_listen(void)
{
	...
	c = pa_context_new(mlapi, "evblocks");
	assert(c);
